...
attr_accessor :player_guess
  ...
  def wrong_guess(right_positions, unguessed_secret)
    ...
    def feedback(right_positions, sum)
      p "'#{@player_guess.upcase}' has #{sum} of the correct elements with #{right_positions} in the correct positions. You've taken #{@number_of_guesses} guess."
      guess_again
    end
    ...
    def end_of_game(minutes, seconds)
      p "Congratulations! You guessed the sequence #{@secret} in #{@number_of_guesses} guesses over #{minutes} minutes, #{seconds} seconds."
      play_again_prompt
    end
